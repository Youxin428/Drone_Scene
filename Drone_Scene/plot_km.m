...
drone_Y = 4000:4000:40000; 
distance_drone = zeros(1,length(drone_Y));

TDOA_MSE = zeros(1,length(drone_Y));
MPR_MSE = zeros(1,length(drone_Y));
DAD_MSE = zeros(1,length(drone_Y));

for dis_idx = 1:length(drone_Y)
    drone_loc  = [-500, drone_Y(dis_idx)]; 
    ...
    fprintf('方位（重心） = %.2f °\n', aoa_mean);
    fprintf('距站点距离 = %.1f m\n', dist_real);
    distance_drone(dis_idx) = norm(mean_pos - drone_loc);
    ...
    % TDOA_MSE(dis_idx) = 10*log10(sum(TDOA_est_bias)/K);
    % MPR_MSE(dis_idx) = 10*log10(sum(MPR_est_bias)/K);
    % DAD_MSE(dis_idx) = 10*log10(sum(DAD_est_bias)/K);
    TDOA_MSE(dis_idx) = sqrt(sum(TDOA_est_bias)/K);
    MPR_MSE(dis_idx) = sqrt(sum(MPR_est_bias)/K);
    DAD_MSE(dis_idx) = sqrt(sum(DAD_est_bias)/K);
...
plot(distance_drone/1e3, TDOA_MSE, 'o-', 'color', [0.9290 0.6940 0.1250], 'LineWidth', 1.5, 'MarkerSize', 8); hold on;
plot(distance_drone/1e3, MPR_MSE, 's--', 'color', [0.4940 0.1840 0.5560], 'LineWidth', 1.5, 'MarkerSize', 8);
plot(distance_drone/1e3, DAD_MSE, '^-.', 'color', [0.4660 0.6740 0.1880], 'LineWidth', 1.5, 'MarkerSize', 8);

% 图例、标题、坐标轴
legend({'TDOA-LS', 'SUM-MPR', 'DAD'}, 'FontSize', 12);
title('目标距离对测向性能影响对比图', 'FontSize', 14);
