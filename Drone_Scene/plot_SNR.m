...
%% --- 无人机信号发射参数设置 ---
sig_power_dBm = 20:2:40; 
SNR_receive = zeros(1,length(sig_power_dBm));

TDOA_MSE = zeros(1,length(sig_power_dBm));
MPR_MSE = zeros(1,length(sig_power_dBm));
DAD_MSE = zeros(1,length(sig_power_dBm));

for SNR_idx = 1:length(sig_power_dBm)

    sig_power_W = 10^((sig_power_dBm(SNR_idx) - 30)*0.1);
    ...
    %计算接收信噪比
    noise_dBm = -104.96;%由NSPD直接算出
    for i = rec_num
        d_ij = sqrt((rec_loc(i, 1) - drone_loc(1,1))^2 + (rec_loc(i, 2) - drone_loc(1,2))^2);
        loss = decline_mode(decline_para, 5820e6/1e6, d_ij/1e3);
        SNR_receive(SNR_idx) = sig_power_dBm(SNR_idx) - loss -noise_dBm;
    end
    ...
    for kk = 1:K
        ...
        % [aoa_DAD,r_DAD] = DAD_sphere_search(rcvPos_ori,sig_fft,Fs,0,2,180,1000,100,2000);
    ...
    % TDOA_MSE(SNR_idx) = 10*log10(sum(TDOA_est_bias)/K);
    % MPR_MSE(SNR_idx) = 10*log10(sum(MPR_est_bias)/K);
    % DAD_MSE(SNR_idx) = 10*log10(sum(DAD_est_bias)/K);
    TDOA_MSE(SNR_idx) = sqrt(sum(TDOA_est_bias)/K);
    MPR_MSE(SNR_idx) = sqrt(sum(MPR_est_bias)/K);
    DAD_MSE(SNR_idx) = sqrt(sum(DAD_est_bias)/K);
