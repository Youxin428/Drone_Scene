% 将发射台站序号传入，将所有可能频率分成四组，每个台站的频率只能在本组内取得，故不会碰撞
% 跳速10hop/s
function [FH_data, Fc_rand, finally_message, sig_message, part_long, Fc, Fc_total] = FH_generation_orthogonal(Fs, Fd, SNR, noise_power, nfft, data_long, number, trans_num, Fc)
...
%     Fc = 10e3:20e3:20e6;
fc_num = 200;%500 80
Fc_total = get_fc_point(0,20e6,fc_num);
sig = zeros(1,data_long);
part_sig_len = 200000; % 20000 信号长度0.0004s   4000000 0.08s  200000
part_interval = 50000; % 5000 信号间时域的保护间隔0.0001s  1000000 0.02s  50000
part_data_len = part_sig_len+part_interval;
sig_num = data_long/part_data_len;

% 生成第一个发射台站的信号时把四个台站的频率都一次性生成了
if number == 1
