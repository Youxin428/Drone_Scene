...
self_clock_sigma=1e-9*[(0:10:100)]; %随机同步误差的标准差ns

TDOA_MSE = zeros(1,length(self_clock_sigma));
MPR_MSE = zeros(1,length(self_clock_sigma));
DAD_MSE = zeros(1,length(self_clock_sigma));

for sync_idx = 1:length(self_clock_sigma)
    sync_error = self_clock_sigma(sync_idx) * randn(rec_num,1); %每个站的随机同步误差
    ...
    for kk = 1:K
        ...
        for i = 1:drones_num % 循环处理每个无人机
            ...
            for j = 1:rec_num % 循环处理每个接收站接收到的信号
                ...
                % 2. 计算时延
                td(i, j) = d_ij / c + sync_error(j); %引入同步误差
        ...
        fprintf('方位（东偏北） = %.2f °\n', aoa_DAD);
        % fprintf('距站点距离 = %.1f m\n', r_DAD);
        DAD_est_bias(kk) = (aoa_DAD-aoa_real)^2;

        rec_loc = [[600,500];[1000,100];[500,-700];[-400,-600]];
    end
    TDOA_MSE(sync_idx) = 10*log10(sum(TDOA_est_bias)/K);
    MPR_MSE(sync_idx) = 10*log10(sum(MPR_est_bias)/K);
    DAD_MSE(sync_idx) = 10*log10(sum(DAD_est_bias)/K);
    % TDOA_MSE(sync_idx) = (sum(TDOA_est_bias)/K);
    % MPR_MSE(sync_idx) = (sum(MPR_est_bias)/K);
    % DAD_MSE(sync_idx) = (sum(DAD_est_bias)/K);

end
% 画图
figure;

plot(self_clock_sigma, TDOA_MSE, 'o-', 'color', [0.9290 0.6940 0.1250], 'LineWidth', 1.5, 'MarkerSize', 8); hold on;
plot(self_clock_sigma, MPR_MSE, 's--', 'color', [0.4940 0.1840 0.5560], 'LineWidth', 1.5, 'MarkerSize', 8);
plot(self_clock_sigma, DAD_MSE, '^-.', 'color', [0.4660 0.6740 0.1880], 'LineWidth', 1.5, 'MarkerSize', 8);

% 图例、标题、坐标轴
legend({'TDOA-LS', 'SUM-MPR', 'DAD'}, 'FontSize', 12);
title('时钟同步误差对测向性能影响对比图', 'FontSize', 14);
xlabel('σ', 'FontSize', 12);
