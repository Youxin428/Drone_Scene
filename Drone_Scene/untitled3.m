function [downlink_signal, used_Fc_sequence, waterfall_message, signal_parameters, ...
                                            waterfall_segment_long, rf] = generateDownlinkSignal(Fs, sig_power, nfft, data_long)
% --- 定义跳频频率池生成 ---
...
part_sig_len = round(0.01 * Fs);            % 有效信号长度 (~0.01秒)
part_interval_len = round(0.003 * Fs);  % 保护间隔长度 (~0.003秒)
part_data_len = part_sig_len + part_interval_len;
sig_num = floor(data_long/part_data_len);
repeat_factor = floor(0.1 * Fs / part_data_len);
Fc_pool = get_fc_point(target_freq_start, target_freq_end, bandwidth_options, fc_num);
...
waterfall_message = tf_change(signal_parameters, part_long, nfft, Fs); % 修正: finally_message -> waterfall_message
noverlap = 0;
